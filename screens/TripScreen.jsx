...
export default function TripScreen({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      setIsLoad(true);
      try {
        const tripFetch = await fetch(
          `${BACK_URL}/trips/${tokenTrip}?token=${user.token}`,
        );
  ...
  const eventsTrip = events.filter(
    (event) =>
      event.tokenTrip === trip.tokenTrip &&
      compareDesc(new Date(event.date), currentDate) === 0,
  );
  ...
  const uriBackground =
    'https://res.cloudinary.com/djjyzmssb/image/upload/v1691664601/background_x2cjvc.png';
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.innerContainer}>
          <View style={styles.calendarContainer}>
            ...
            >
              <TouchableOpacity
                style={styles.flecheLeft}
                onPress={onPreviousDate}
              >
                <SvgArrow
                  width={60}
                  height={60}
                  fill={GLOBAL_COLOR.PRIMARY}
                  direction="left"
                />
              </TouchableOpacity>
