...
export default function NewTripScreen({ route, navigation }) {
  ...
  const [endDateText, setEndDateText] = useState(
    trip.dateEnd ? format(new Date(trip.dateEnd), "dd'/'MM'/'yyyy") : '',
  );
    ...
    (async () => {
      try {
        const fetchUsers = await fetch(
          `${BACK_URL}/users/list?token=${user.token}`,
        );
  ...
  return (
    <>
      ...
      <SafeAreaView style={styles.screen}>
        ...
        >
          <HeaderNav
            title={trip.tokenTrip ? 'Mettre à jour le Trip' : 'Nouveau Trip'}
          ...
          <ScrollView style={styles.content}>
            ...
            <TouchableOpacity style={styles.btnBuddy} onPress={handleModal}>
              <Text style={styles.buddyText}>
                Sélectionner des buddies -{' '}
                <Text style={{ fontWeight: 'bold' }}>
                  {buddiesSelected.length}/50
                </Text>
              </Text>
            </TouchableOpacity>
