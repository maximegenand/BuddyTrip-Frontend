...
export const tripsSlice = createSlice({
  ...
  reducers: {
    updateTrip: (state, action) => {
      const newTripsList = state.value.map((data) =>
        data.tokenTrip !== action.payload.tokenTrip ? data : action.payload,
      );
    ...
    deleteTrip: (state, action) => {
      state.value = state.value.filter(
        (trip) => trip.tokenTrip !== action.payload,
      );
    },
  },
});

export const { updateTrip, addAllTrips, addTrip, deleteTrip } =
  tripsSlice.actions;
